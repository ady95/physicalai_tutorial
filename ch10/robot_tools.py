"""10부 — LLM에게 쥐여 줄 로봇 도구 계층.

3부에서 만든 SO101Sim 위에 얇게 한 겹 올린 것뿐입니다. 새로 만드는 제어 코드는 없습니다.
LLM은 "무엇을 할지"만 정하고, IK와 관절 보간은 3부 코드가 그대로 합니다. 이 분리가 10-1의 핵심입니다.
...
# 이 값을 빼고 모든 이동을 1초로 고정하면, 상자까지의 긴 대각선 이동에서 관성으로 블록을 놓친다 (10-2).
MOVE_SPEED = 0.15                 # m/s
...
class RobotTools:
    ...
    def _t_move_gripper_to(self, x, y, z):
        ...
        # LLM이 판단에 쓸 수 있도록 "얼마나 못 갔는가"와 "손가락이 기울었는가"를 나눠서 돌려준다 (10-3).
